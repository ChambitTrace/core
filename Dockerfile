...
# libbpf (submodule)
RUN cd /app/external/libbpf/src && make && make install && make BUILD_STATIC_ONLY=y
...
RUN chmod +x /app/src/trace/loader
RUN chmod +x /app/src/trigger/main

# 실행 전 trigger 디렉토리로 이동
WORKDIR /app/src/trigger
